...
tar -zcvf - /Volumes/Brian/Users/ibrianallen --exclude-from="$DOTFILES/common/users/timotheos/conf/rclone/exclude-brian.txt" |
  rclone --dry-run --progress --stats 2s --verbose 2 --log-file="$HOME/Library/Logs/rclone-testing.log" --fast-list \
  rcat --dump filters --transfers 100 --checkers 200 aws_gda-crypt:/brian_macos_10.11.6.tar.gz | tee ~/Library/Logs/rclone-stdout.log
