      ...
      "rclone.conf" = {
        content = # bash
          ''
            ...
            type = s3
            provider = AWS
            access_key_id = ${config.sops.placeholder."users/${user1}/rclone_keys/rclone_aws_gda_keyid"}
            secret_access_key = ${config.sops.placeholder."users/${user1}/rclone_keys/rclone_aws_gda_keysecret"}
            region = us-east-1
            acl = private
