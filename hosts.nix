...
let
  ...
  # -------------------------------------------------------------
  nixosHosts = {
    oona  = { system = "x86_64-linux"; channel = "nixpkgs"; };
    lady  = { system = "x86_64-linux"; channel = "nixpkgs"; };
    blue  = { system = "x86_64-linux"; channel = "nixpkgs-26_05"; };
    remus = { system = "x86_64-linux"; channel = "nixpkgs-26_05"; };
  };

  darwinHosts = {
    mack = { system = "x86_64-darwin"; };
  };

  initHosts = {
    # Nixos-anywhere bootstrap environment targets
    remus-init = { system = "x86_64-linux"; channel = "nixpkgs-26_05"; targetHost = "remus"; username = "timotheos"; };
  };
  ...
  sharedArgs = system: hostname: {
    inherit hostname inputs self system;
...
in {
  ...
  flake.nixosConfigurations = (builtins.mapAttrs (hostname: hostAttrs:
    inputs."${hostAttrs.channel}".lib.nixosSystem {
      ...
      modules = [
        ...
        inputs."home-manager${if hostAttrs.channel == "nixpkgs" then "" else "-26_05"}".nixosModules.home-manager
      ];
    }
  ) nixosHosts) 
  
  # --- 🌟 Bootstrapping NixOS Profiles (initNixos replacement) ---
  // (builtins.mapAttrs (initName: initAttrs:
  ...
  flake.darwinConfigurations = builtins.mapAttrs (hostname: hostAttrs:
