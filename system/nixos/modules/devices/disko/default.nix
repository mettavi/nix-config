...
in
{
  ...
  config = mkIf cfg.enable {
    disko.devices.disk = mapAttrs (disk: diskCfg: {
      ...
      content = {
        type = "${diskCfg.content.partScheme}";
        partitions = mapAttrs (
          part: partCfg:
          let
            cfg = config.mettavi.system.devices.disko;
          ...
          {
            label =
              optionalString (cfg.disks.${disk}.content.partitions.${part}.label != "")
                "${partCfg.label}";
            name = "${partCfg.name}";
            size = optionalString (cfg.disks.${disk}.content.partitions.${part}.size != "") "${partCfg.size}";
            type = optionalString (
              cfg.disks.${disk}.content.partitions.${part}.partType != ""
            ) "${partCfg.partType}";
            content =
              let
                partContent = cfg.disks.${disk}.content.partitions.${part}.content;
              in
              {
                type = optionalString (partContent.contentType != "") "${partContent.contentType}";
                ...
                ) cfg.disks.${disk}.content.partitions.${part}.content.btrfsSubs;
              };
          }
        ) cfg.disks.${disk}.content.partitions;
      };
    }) cfg.disks;
  };
}
