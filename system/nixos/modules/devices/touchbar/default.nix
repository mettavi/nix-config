...
with lib;
let
  cfg = config.mettavi.system.devices.touchbar;
in
{
  options.mettavi.system.devices.touchbar = {
    enable = mkOption {
      type = types.bool;
      default = false;
      description = "Set up and configure macOS touchbar";
    };
  };

  config = mkIf cfg.enable {
    hardware.apple.touchBar = {
      # enable support for the Touch Bar using tiny-dfr
      enable = true;
      settings = {
